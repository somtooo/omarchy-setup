...
Item {
  ...
  component BarPanel: PanelWindow {
    ...
    margins {
      top: root.barHidden && root.position === "top" ? -(root.barSize + pillTopMargin + pillShadowPad) : 0
      bottom: root.barHidden && root.position === "bottom" ? -(root.barSize + pillTopMargin + pillShadowPad) : 0
      left: root.barHidden && root.position === "left" ? -(root.barSize + pillMargin + pillShadowPad) : 0
      right: root.barHidden && root.position === "right" ? -(root.barSize + pillMargin + pillShadowPad) : 0
    }
    ...
    // Floating pill: window is a touch larger than the pill so the side/top
    // margins and the drop shadow have room without being clipped.
    readonly property int pillMargin: Style.space(4)
    readonly property int pillTopMargin: Style.space(4)
    readonly property int pillShadowPad: Style.space(10)
    implicitWidth: root.vertical ? root.barSize + pillMargin + pillShadowPad : 0
    implicitHeight: root.vertical ? 0 : root.barSize + pillTopMargin + pillShadowPad
    color: "transparent"
    surfaceFormat.opaque: false
    WlrLayershell.namespace: "omarchy-bar"
    WlrLayershell.layer: WlrLayer.Top

    // Frosted pill background. Opaque theme color when not transparent,
    // translucent dark glass when transparent (double-tap toggle preserved).
    Rectangle {
      id: pill
      ...
      height: root.barSize
      anchors.margins: 0
      anchors.leftMargin: barWindow.pillMargin
      anchors.rightMargin: barWindow.pillMargin
      anchors.topMargin: root.barHidden ? 0 : barWindow.pillTopMargin
      radius: height / 2
      color: root.transparent ? Qt.rgba(0.06, 0.06, 0.10, 0.45) : Qt.rgba(0.05, 0.05, 0.09, 0.62)
      border.width: 1
      border.color: root.transparent ? Qt.rgba(1, 1, 1, 0.12) : Qt.rgba(1, 1, 1, 0.08)
    ...
    Loader {
      ...
      anchors.leftMargin: barWindow.pillMargin
      anchors.rightMargin: barWindow.pillMargin
      anchors.topMargin: barWindow.pillTopMargin
